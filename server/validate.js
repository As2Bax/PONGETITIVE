...
/* Must equal MAX_BOUNCE_ANGLE in app/js/core.js. The simulation clamps aim to
   this range itself; duplicating it here means a hand-crafted packet is
   rejected at the edge rather than trusted into the sim. */
const MAX_AIM_ANGLE = Math.PI / 3.4;
...
export function sanitizeInput(payload) {
  ...
  const am = Number(payload.am);
  return {
    t: 'i',
    // A non-finite or absent aim means "no change", not a teleport to NaN.
    y: Number.isFinite(y) ? y : null,
    /* Held-ball aim angle. A BOUNCE angle in the same convention paddleBounce
       and releaseBall use: signed vertical deflection relative to the holding
       paddle's own facing, never an absolute heading. The simulation applies
       direction from whichever side holds the ball, so the value needs no
       mirroring between host and guest.

       Clamping here is the anti-cheat, in the same spirit as MAX_PADDLE_SPEED:
       the client asks for an angle, the server refuses anything a legitimate
       player could not aim. Absent means "no change". */
    am: Number.isFinite(am) ? Math.max(-MAX_AIM_ANGLE, Math.min(MAX_AIM_ANGLE, am)) : null,
